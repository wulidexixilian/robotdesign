...
massPara = [
            ...
            # arm structure
            {
             "nest":"arm", "cm":np.array([63.6, -13.5, -50.2])*1e-3, "m":0,
             "iT":np.array([12423, 30203, 29545, 235, -317, -207])*1e-6
            },
            # arm cabel assembly
            {
             "nest":"arm", "cm":np.array([60, -3, -85])*1e-3, "m":0,
             "iT":np.array([120, 600, 650, 0, 90, -42])*1e-6
            },

            # wrist structure
            {
             "nest":"handbase", "cm":np.array([0, 2.5, -101.45])*1e-3, "m":0,
             "iT":np.array([13348, 12263, 6332, -1195, 3.4, -1])*1e-6
            },
            # wrist cable
            {
             "nest":"handbase", "cm":np.array([7, -30, -104])*1e-3, "m":0,
             "iT":np.array([36, 304, 104, -85, 0, 0])*1e-6
            },

            # wrist hand structure
            {
             "nest":"handwrist", "cm":np.array([-1.2, 0.1, 5.8])*1e-3, "m":0,
             "iT":np.array([464, 507, 528, 0, -13, -0.1])*1e-6
            },
            # wrist hand installation material + cable
            {
             "nest":"handwrist", "cm":np.array([11.5, 12, -6.8])*1e-3, "m":0,
             "iT":np.array([126, 100, 121, 10, -64, -11.9])*1e-6
            },
            # wrist hand adapter
            {
             "nest":"handwrist", "cm":np.array([48.8, 0, 0])*1e-3, "m":0,
             "iT":np.array([87, 43, 45, 0, 0, 0])*1e-6
            },
            # wrist hand other
            {
             "nest":"handwrist", "cm":np.array([28.3, 0.4, 6])*1e-3, "m":0,
             "iT":np.array([126, 163, 130, 1.1, -15.1, -2.2])*1e-6
            },
            # flange structure
            {
             "nest":"handflange", "cm":np.array([0, 0, -1.4])*1e-3, "m":0,
             "iT":np.array([81, 81, 155, 0, 0, 0])*1e-6
            },
            # flange screws
            {
             "nest":"handflange", "cm":np.array([0, 0, 1])*1e-3, "m":0,
